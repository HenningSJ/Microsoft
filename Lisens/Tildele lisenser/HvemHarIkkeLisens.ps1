...
# Hent alle brukere i tenanten (kan ta tid hvis mange brukere)
$allUsers = Get-MgUser -All

# Opprett liste for de som mangler lisensen
$missingLicense = @()

foreach ($user in $allUsers) {
    try {
        $licenses = Get-MgUserLicenseDetail -UserId $user.Id
        if (-not ($licenses.SkuId -contains $skuId)) {
            $missingLicense += [PSCustomObject]@{
...
$missingLicense | Export-Csv "C:\Temp\UsersWithoutF1EEA.csv" -NoTypeInformation -Encoding UTF8
